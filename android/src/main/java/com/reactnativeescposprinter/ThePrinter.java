...
public class ThePrinter implements StatusChangeListener, PrinterSettingListener, ReceiveListener {
     ...
     *                      returns void
     */
    synchronized public void setupWith(final String printerTarget, final int series, final int lang,
            final PrinterDelegate callback) throws Epos2Exception {
    ...
    /**
     * Returns void
     * Function removeDelegates removes all delegate callbacks. Used when tring to
     * remove object
     */
    ...
    synchronized public boolean isPrinterBusy() {
        ...
        if (connectingState_ != ThePrinterState.PRINTER_IDLE)
            isBusy = true;
    ...
    public void onPtrStatusChange(Printer printer, int eventType) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                synchronized (delegateSync_) {
                    if (delegate_ != null)
                        delegate_.onPrinterStatusChange(_objID, eventType);
                }
            }
        }).start();
    ...
    public void onGetPrinterSetting(int code, int type, int value) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                synchronized (delegateSync_) {
                    if (delegate_ != null)
                        delegate_.onGetPrinterSetting(_objID, code, type, value);
                }
                isWaitingForPrinterSettings_ = false;
            }
        }).start();
